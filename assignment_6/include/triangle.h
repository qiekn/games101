...
public:
  ...
  bool Intersect(const Ray& ray, float& tnear, uint32_t& index) const {
    ...
    for (uint32_t k = 0; k < num_triangles_; ++k) {
      const Vector3f& v0 = vertices[vertex_index_[k * 3]];
      const Vector3f& v1 = vertices[vertex_index_[k * 3 + 1]];
      const Vector3f& v2 = vertices[vertex_index_[k * 3 + 2]];
      float t, u, v;
  ...
  void GetSurfaceProperties(const Vector3f& P, const Vector3f& I, const uint32_t& index, const Vector2f& uv,
    ...
    const Vector3f& v0 = vertices[vertex_index_[index * 3]];
    const Vector3f& v1 = vertices[vertex_index_[index * 3 + 1]];
    const Vector3f& v2 = vertices[vertex_index_[index * 3 + 2]];
  ...
  std::unique_ptr<uint32_t[]> vertex_index_;
  std::unique_ptr<Vector2f[]> st_coordinates_;
