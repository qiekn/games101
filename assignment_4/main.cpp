...
cv::Point2f recursive_bezier(const std::vector<cv::Point2f> &control_points, float t) {
  // INFO: Implement de Casteljau's algorithm
  ...
  if (remain_points.size() == 1) return remain_points[0];
  return cv::Point2f();
}

void bezier(const std::vector<cv::Point2f> &control_points, cv::Mat &window) {
  // INFO: Iterate through all t = 0 to t = 1 with small steps, and call de Casteljau's
